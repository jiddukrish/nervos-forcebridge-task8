...
const SUDT_ID = '2066';
const SUDT_PROXY_CONTRACT_ADDRESS = '0xc7B4f6256fbA89795447dc1C89226e27cB2880a6';
const FORCE_BRIDGE_ETH =
...
export function App() {
    ...
    const [sudtBalance, setSudtBalance] = useState();
    const [depositAddress, setDepositAddress] = useState<string>();
    useEffect(() => {
        if (accounts?.[0]) {
            const addressTranslator = new AddressTranslator();
            setPolyjuiceAddress(addressTranslator.ethAddressToGodwokenShortAddress(accounts?.[0]));
            getL2DepositAddress(addressTranslator);
        } else {
            setPolyjuiceAddress(undefined);
        }
    }, [accounts?.[0]]);

    const getL2DepositAddress = async (addressTranslator: AddressTranslator) => {
    ...
    useEffect(() => {
        if (accounts && web3 && polyjuiceAddress) {
            getSUDTBalance();
        }
    }, [accounts, web3, polyjuiceAddress]);
    ...
    const getSUDTBalance = async () => {
        ...
        const _balanceOf = await _contract.methods.balanceOf(polyjuiceAddress).call({
            from: accounts?.[0]
        });
    ...
    return (
        <div className="main">
            ...
            <div className="account-info">
                ...
                <br />
                SUDT Token Balance:{' '}
                <b>{sudtBalance ? (sudtBalance as any) : <LoadingIndicator />} SUDT</b>
                <br />
                <br />
                <div className="deposit-address">
                    Layer2 Deposit Address:{' '}
                    <b> {depositAddress ? (depositAddress as string) : <LoadingIndicator />}</b>
                    <hr />
                </div>
            </div>

            <div className="force-bridge">
                <h2>How to Deposit?</h2>
                <p>
                    Copy your layer2 deposit address and go to force bridge to deposit ETH into your
                    Nervos account
                </p>

                <button
                    onClick={() => {
                        window.open(FORCE_BRIDGE_ETH, '_blank');
                    }}
                >
                    Force Bridge
                </button>
